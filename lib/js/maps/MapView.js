...
export default class MapView extends Component {
  ...
  // react-native-maps
  animateCamera(target, duration = 500) {
    if (target.zoom) {
      target.zoomLevel = target.zoom;
    }
    if (target.center) {
      target.coordinate = target.center;
    }
    this.sendCommand("animateTo", [target, duration]);
  }
  ...
  _onRegionChangeComplete = (e) => {
    this.props.onRegionChangeComplete && this.props.onRegionChangeComplete(e.nativeEvent);
  }
  ...
  render() {
    ...
    // react-native-maps
    if (props.camera) {
      props.coordinate = props.camera.center;
      props.zoomLevel = props.camera.zoom;
    }
    if (props.onRegionChange) {
      props.onStatusChange = this._onRegionChange;
    }
    if (props.onRegionChangeComplete) {
      props.onStatusChangeComplete = this._onRegionChangeComplete;
    }
    if (props.pitchEnabled != null) {
      props.tiltEnabled = props.pitchEnabled;
    }
    // react-native-maps end
    ...
    if (props.coordinate) {
      if (props.coordinate.latitude == null || props.coordinate.longitude == null) {
