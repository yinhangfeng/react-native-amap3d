...
export default class MapView extends Component<any> {
  ...
  _onPress = (e) => {
    // console.log('mapview onPress', e.nativeEvent)
    if (e.nativeEvent.longitude != null) {
      // 目前测试 当点击在 marker 上的时候也会触发 onPress 但不会带有 longitude latitude
      this._setActiveMarkerIOS(null);
    }
    const onPress = this.props.onPress;
    onPress && onPress(e);
  };

  _setActiveMarkerIOS(activeMarker) {
    if (activeMarker === this._activeMarker) {
      return;
    }
    if (this._activeMarker) {
      this._activeMarker._setActiveIOS(false);
    }
    this._activeMarker = activeMarker;
    if (activeMarker) {
      activeMarker._setActiveIOS(true);
    }
  }
  ...
  render() {
    ...
    // 允许 coordinate region 从有值到空 (内部保持原坐标)
    if (!props.coordinate) {
      if (this._prevCoordinate) {
        // 创建 _prevCoordinateCopy 防止 coordinate 内部包含其它数据引起内存泄漏
        this._prevCoordinateCopy = {
          latitude: this._prevCoordinate.latitude,
          longitude: this._prevCoordinate.longitude,
        };
        this._prevCoordinate = null;
      }
      if (this._prevCoordinateCopy) {
        props.coordinate = this._prevCoordinateCopy;
      } else {
        props.coordinate = undefined;
      }
    } else {
      this._prevCoordinate = props.coordinate;
    }

    if (!props.region) {
      if (this._prevRegion) {
        this._prevRegionCopy = {
          latitude: this._prevRegion.latitude,
          longitude: this._prevRegion.longitude,
          latitudeDelta: this._prevRegion.latitudeDelta,
          longitudeDelta: this._prevRegion.longitudeDelta,
        };
        this._prevRegion = null;
      }
      if (this._prevRegionCopy) {
        props.region = this._prevRegionCopy;
      }
    } else {
      this._prevRegion = props.region;
    }
    ...
    if (Platform.OS === 'ios') {
      ...
      // Marker 需要 mapViw 实例
      const args = React.Children.map(props.children, (child) => {
        if (!child) {
          return child;
        }
