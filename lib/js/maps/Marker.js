...
export default class Marker extends Component {
  ...
  componentDidMount() {
    if (Platform.OS === 'ios') {
      ...
      if (__DEV__ && !_mapView) {
        console.warn('自定义 Marker 时 必须传入 _mapView props');
        return;
      }
      if (_mapView && active && !_mapView._activeMarker) {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (Platform.OS === 'ios') {
      ...
      if (_mapView && !prevProps.active && active && this.state.activeIOS == null && !_mapView._activeMarker) {
        _mapView._activeMarker = this;
      }
    }
  }
  ...
  componentWillUnmount() {
    ...
    if (Platform.OS === 'ios' && this.props._mapView && this.props._mapView._activeMarker === this) {
  ...
  render() {
    ...
    // 对非法 coordinate 的容错机制
    if (!otherProps.coordinate || otherProps.coordinate.latitude == null || otherProps.coordinate.longitude == null) {
      if (__DEV__) {
        console.warn('Marker coordinate null', this.props);
      }
      otherProps.coordinate = {
        latitude: 90,
        longitude: 0,
      };
    }

    if (otherProps.infoWindowDisabled == null && otherProps.title == null) {
    ...
    if (icon) {
      ...
      // customMarker = <View collapsable={false} style={styles.overlay}>{icon()}</View>;
      // if (children) {
      ...
      if (customMarker) {
        customMarker = <View collapsable={false} style={styles.overlay}>{customMarker}</View>;
      }
    }
